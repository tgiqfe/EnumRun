    ...
    internal class ExecSession2
    {
        ...
        public static Result Check(EnumRunSetting setting)
        {
            ...
            if (lastSession != null)
            {
                ...
                //  実行時間
                if (((DateTime)lastSession.ExecTime - (DateTime)currentSession.ExecTime).TotalSeconds <= setting.RestTime)
                {
                    ret.Runnable = false;
            ...
            //  FilesPathの有無チェック
            if (!File.Exists(currentSession.FilesPath))
            {
                ret.Runnable = false;
